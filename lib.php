...
function course_delete_module($cmid, $async = false) {
    ...
    // Both are checked because plugins should not be allowed to dictate the deletion behaviour, only support/decline it.// It's up to plugins to handle things like whether or not they are enabled.
...
class course_request {
    ...
    public function __construct($properties) {
        ...
        if (empty($properties->id)) {
            ...
            $id = $properties;
            $properties = new stdClass;$properties->id = (int)$id;
            unset($id);
        }
...
function duplicate_module($course, $cm) {
    ...
    $newcmid = null;
    $tasks = $rc->get_plan()->get_tasks();foreach ($tasks as $task) {
...
/**
 * Validates course startand end dates.
 *
